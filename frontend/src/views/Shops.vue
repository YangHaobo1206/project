...
const save = async () => {
  ...
  saving.value = true
  try {
    const payload = { name: form.name, description: form.description, address: form.address }
    if (form.id) {
...
const toggle = async (row) => {
  const action = row.online ? 'offline' : 'online'
  await http.post(`/api/shops/${row.id}/${action}`)
